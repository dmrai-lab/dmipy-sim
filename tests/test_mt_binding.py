...
def test_emergent_bound_fraction(k_f, k_r):
    ...
    _, bfrac = simulate_bloch(8000, D, replace(_zero_wf(n_t, dt), rf=EXC), Sphere(radius=R), kappa_MT=kappa_MT, dwell_time=dwell, seed=3, return_bound_frac=True)
...
def test_binding_timestep_independence():
    ...
    def run(nsub):
        _, bf = simulate_bloch(6000, D, replace(_zero_wf(n_t, dt), rf=EXC), Sphere(radius=R), kappa_MT=kappa_MT, dwell_time=dwell, seed=3, sub_steps=nsub, return_bound_frac=True)
...
def test_transverse_exchange_matches_oracle():
    ...
    S_mt = np.abs(simulate_bloch(12000, D, replace(_zero_wf(n_t, dt), rf=EXC, readout=echo), geom, T2=T2a, kappa_MT=kappa_MT, dwell_time=dwell, T2_bound=T2b, T1_bound=1.0, seed=5, equilibrate_binding='off')[0])
    S0 = np.abs(simulate_bloch(12000, D, replace(_zero_wf(n_t, dt), rf=EXC, readout=echo), geom, T2=T2a, seed=5)[0])       # no MT -> plain T2a
    ratio_mc = S_mt / S0
