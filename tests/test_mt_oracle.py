...
def test_equilibrate_binding_is_named():
    ...
    assert [mt.resolve_equilibrate_mode(m, g) for m in ("off", "burnin")] == ["off", "burnin"]
    for bad in (None, False, True, 0, 1, "auto"):
        with pytest.raises(ValueError, match="'burnin'\\|'fast'\\|'off'"):
