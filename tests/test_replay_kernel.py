...
def _synthetic_trajectories(n_t=N_T, dt=DT):
    """(name, traj (n_w, n_t, 3), closed-form phase (n_w,)) for the bipolar waveform of `_bipolar`."""
    t = np.arange(n_t) * dt
    g = _bipolar(n_t)[0, :, 0]
    out = []
    # stationary: phi = gamma dt x0 sum_t g = 0 (refocused)
    x0 = np.array([1e-6, -2e-6, 3e-6])
    traj = np.zeros((3, n_t, 3)); traj[:, :, 0] = x0[:, None]
    out.append(("stationary", traj, GAMMA * dt * x0 * g.sum()))
    # constant velocity: phi = gamma dt v sum_t t g(t)
    v = np.array([1e-3, -5e-4])
    traj = np.zeros((2, n_t, 3)); traj[:, :, 0] = v[:, None] * t[None, :]
    out.append(("constant velocity", traj, GAMMA * dt * v * (t * g).sum()))
    # one sine mode: phi = gamma dt A sum_t sin(pi t / T) g(t)
    A = np.array([2e-6, 7e-7])
    mode = np.sin(np.pi * t / T_TOTAL)
    traj = np.zeros((2, n_t, 3)); traj[:, :, 0] = A[:, None] * mode[None, :]
    out.append(("sine mode", traj, GAMMA * dt * A * (mode * g).sum()))
    return out
...
def test_every_route_returns_the_closed_form_phase(case):
    ...
    routes["kernel"] = gradient_phase(resample_gradient(G, DT, DT, N_T), traj, DT)[0]
    ...
    routes["increments"] = phase_increments(G[0], traj, DT).sum(0)
...
def test_jax_routes_agree_with_numpy():
    ...
    phi_j = np.asarray(gradient_phase_jax(resample_gradient_jax(jnp.asarray(G), DT, DT, N_T),
    ...
    inc = np.asarray(phase_increments_jax(G[0], traj, DT)).sum(0)
...
def test_resampling_is_one_rule():
    ...
    padded = resample_gradient(G, DT, DT, 2 * N_T)
    assert padded.shape == (1, 2 * N_T, 3) and np.all(padded[:, N_T:] == 0)
    fine = resample_gradient(G, DT, DT / 4, 4 * (N_T - 1) + 1)
    assert fine.shape == (1, 4 * (N_T - 1) + 1, 3)
    np.testing.assert_allclose(fine[0, ::4, 0], G[0, :, 0])            # exact at the nodes
    assert fine[0, 1, 0] == pytest.approx(G0)                          # inside a lobe: constant
    beyond = resample_gradient(G, DT, DT, N_T)
    assert np.all(resample_gradient(G, DT, DT / 2, 3 * N_T)[0, 2 * N_T:, 0] == 0)   # past the window
    del beyond
...
def test_compiled_scheme_and_mode_space_read_one_projection():
    ...
    P = cx.bridge_projection(G, N_T, K)
...
def test_one_spin_echo_gate():
    ...
    assert s.sum() == 0 and set(np.unique(s)) <= {-1.0, 0.0, 1.0}
    assert (s[: N_T // 4] == 1).all() and (s[-N_T // 4:] == -1).all()
    np.testing.assert_array_equal(se_gate(N_T, DT, None), np.ones(N_T))
