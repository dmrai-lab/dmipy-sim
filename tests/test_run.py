...
def test_every_long_producer_opens_a_run():
    ...
    for f in (core.simulate, core.simulate_cpmg, core.simulate_trajectories, adaptive.simulate_trajectories_adaptive,
              ...
              StrandFieldBasis.build_far_grid, ReplayPhantom.replay, ReplayPhantom.replay_train, ReplayPhantom.replay_bloch,
              ReplayPack.pose_responses):
...
def test_the_budget_guard_stops_a_run_before_the_host_does(tmp_path, monkeypatch):
    ...
    stopped at a batch boundary -- after the warm-up batches, from the median pace -- with the projection in its
    record, and the pieces it spooled are intact."""
    import gc
    monkeypatch.setenv("DMIPY_SIM_MEMORY_CEILING_BYTES", str(R._rss() + 2500 * 2 ** 20))
    ...
    with pytest.raises(R.ResourceBudgetError, match="memory budget"):
        with R.Run("hog", run_dir=str(tmp_path / "h")) as r:
            for b, (s, e) in enumerate(r.batches(20, 1)):
    ...
    assert R.BUDGET_WARMUP + R.BUDGET_STRIKES - 1 <= len(spooled) < 20 and all(os.path.isfile(x["path"]) for x in spooled)
    del keep; gc.collect()
