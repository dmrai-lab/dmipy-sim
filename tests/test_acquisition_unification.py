...
def test_both_pgse_families_agree_on_the_physical_gradient():
    """``G`` is the gradient a scanner plays: same-sign lobes, the 180 doing the flip; ``G_eff`` is
    what the phase integral walks. Both PGSE constructors build the same physical gradient and derive
    the same effective one."""
    seq = S.pgse(B, DIRS, DELTA, DELTA_BIG, n_t=N_T, slew_rate=np.inf)
    wf = d.set_b(W.pgse(DELTA, DELTA_BIG, 0.1, DIRS, N_T, slew_rate=np.inf), B)
    np.testing.assert_allclose(np.asarray(seq.G), np.asarray(wf.G), rtol=2e-6, atol=0.0)
    np.testing.assert_allclose(np.asarray(seq.G_eff), np.asarray(wf.G_eff), rtol=2e-6, atol=0.0)
...
def test_a_family_that_declares_no_180_has_one_gradient():
    """Where no pulse is folded (ste, pte: bipolar pairs, an excitation only) ``G_eff`` IS ``G``; where a train is
    declared (cpmg) the two differ and the effective one refocuses."""
    for seq in (S.ste(B, DELTA, DELTA_BIG, n_t=N_T), S.pte(B, [0.0, 0.0, 1.0], DELTA, DELTA_BIG, n_t=N_T)):
    ...
    assert not np.array_equal(np.asarray(cp.G_eff), np.asarray(cp.G)) and cp.refocusing_residual < 1e-6
