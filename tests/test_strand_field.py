...
def _finite_line_sum(cls, ra, rb, P, cutoff):
    ...
    for c, a, b in zip(cls, ra, rb):
        ...
        zA = ((P[:, None, :] - A_[None]) * U[None]).sum(-1); zB = zA - L[None]
        rv = (P[:, None, :] - A_[None]) - zA[..., None] * U[None]; rho = np.maximum(np.linalg.norm(rv, axis=-1), 1e-12)
...
def test_the_bounded_evaluation_is_the_full_superposition():
    ...
    with many segments each, every segment within the cutoff counts with its own finite-line factor."""
