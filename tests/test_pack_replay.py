...
COMPS = Compartments(extra=Pool(T2=0.08, T1=1.0), intra=Pool(T2=0.03, T1=1.2))


@pytest.fixture(scope="module")
def packs():
    walk = d.simulate_trajectories(300, D0, d.Cylinder(2e-6, (0, 0, 1)), 0.01, 5e-4, seed=0, require_gpu=False)
    full = build_replay_pack(walk, id="test/full", compartments=COMPS, K=8, envelope=ENV, license="x", citation="x")
...
def test_gradient_replay_is_the_mode_space_contraction(packs):
    ...
    np.testing.assert_allclose(full.replay(wf, relaxation=False), replay_signal(full, _W(full, wf)), rtol=1e-12)
...
def test_relaxation_applies_the_packs_per_pool_rates(packs):
    ...
    np.testing.assert_allclose(full.replay(wf), ref, rtol=1e-12)
    assert full.replay(wf)[0] < full.replay(wf, relaxation=False)[0]     # T2 costs signal, also at b = 0
    with pytest.raises(ValueError, match="no bulk-relaxation tier"):
        plain.replay(wf, relaxation=True)
    np.testing.assert_allclose(plain.replay(wf), plain.replay(wf, relaxation="auto"))
...
def test_surface_relaxivity_uses_the_recorded_diffusivity(packs):
    ...
    np.testing.assert_allclose(full.replay(wf, rho=rho, relaxation=False), ref, rtol=1e-12)
    np.testing.assert_allclose(full.replay(wf, rho=rho, D=D0, relaxation=False), ref, rtol=1e-12)
...
def test_save_and_load_round_trip(packs, tmp_path):
    ...
    wf = _wf(full.n_t, full.dt)
    np.testing.assert_array_equal(back.replay(wf, rho=1e-5), full.replay(wf, rho=1e-5))
