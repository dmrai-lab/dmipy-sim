...
def test_display_gradient_is_same_sign_while_sim_gradient_is_bipolar():
    ...
    wf = set_b(pgse([[1, 0, 0]], 4e-3, 20e-3, gradient_strengths=0.05, n_t=200), 1.0e9)
...
def test_pgse_display_lobes_share_polarity():
    ...
    have opposite signs."""
    wf = pgse([[1, 0, 0]], 4e-3, 20e-3, gradient_strengths=0.05, n_t=200)
...
def test_pgste_storage_window_is_shaded():
    ...
    wf_ste = pgste([[1, 0, 0]], 4e-3, 20e-3, gradient_strengths=0.05, n_t=200)
    ...
    wf_se = pgse([[1, 0, 0]], 4e-3, 20e-3, gradient_strengths=0.05, n_t=200)
    fig, ax = plt.subplots()
...
def test_plot_helpers_run_on_pgse_and_pgste():
    ...
    wf_se = set_b(pgse([[1, 0, 0]], 4e-3, 20e-3, gradient_strengths=0.05, n_t=200), 1.0e9)
    wf_ste = set_b(pgste([[1, 0, 0]], 4e-3, 20e-3, gradient_strengths=0.05, n_t=200), 1.0e9)
...
def test_a_sequence_is_drawn_with_the_physical_gradient_too():
    ...
    seq = S.pgse([[1, 0, 0]], 4e-3, 20e-3, bvalues=[1.0e9], n_t=200)
