...
from dmipy_sim import simulate, Sphere, Cylinder, Mesh, set_b, pgse, pgste, Compartments, Pool
...
def test_side_dependent_surface_relaxivity():
    ...
    s_dict = np.asarray(simulate(3000, D, wf, Mesh(V, F, compartments=Compartments(intra=Pool(surface_relaxivity_t2=rho),
                                                   extra=Pool(surface_relaxivity_t2=rho))), seed=SEED))
    npt.assert_array_equal(s_scalar, s_dict)
    s_in = np.asarray(simulate(3000, D, wf, Mesh(V, F, compartments=Compartments(intra=Pool(surface_relaxivity_t2=rho),
                                                 extra=Pool(surface_relaxivity_t2=0.0))), seed=SEED))
    s_ex = np.asarray(simulate(3000, D, wf, Mesh(V, F, compartments=Compartments(intra=Pool(surface_relaxivity_t2=0.0),
                                                 extra=Pool(surface_relaxivity_t2=rho))), seed=SEED))
...
def test_per_compartment_t2():
    ...
    s_comp = np.asarray(simulate(4000, D, wf, Mesh(V, F, compartments=Compartments(intra=Pool(T2=0.05),
                                                   extra=Pool(T2=0.05))), seed=SEED))
    npt.assert_array_equal(s_global, s_comp)
    s_short = np.asarray(simulate(4000, D, wf, Mesh(V, F, compartments=Compartments(intra=Pool(T2=0.02),
                                                    extra=Pool(T2=0.20))), seed=SEED))
...
def test_per_compartment_t1_pgste_gating():
    ...
    s_t1 = np.asarray(simulate(3000, D, wf_ste, Mesh(V, F, compartments=Compartments(intra=Pool(T1=0.3), extra=Pool(T1=3.0))), seed=SEED))
    ...
    a = np.asarray(simulate(3000, D, wf_se, Mesh(V, F, compartments=Compartments(intra=Pool(T1=0.3), extra=Pool(T1=3.0))), seed=SEED))
...
def test_per_compartment_diffusivity():
    ...
    V, F = _ico(4)
    wf = _pgse(5, 250, TE=40e-3)
    s1 = np.asarray(simulate(4000, None, wf, Mesh(V, F, compartments=Compartments(intra=Pool(D=1e-9), extra=Pool(D=3e-9))), seed=SEED))
    s2 = np.asarray(simulate(4000, None, wf, Mesh(V, F, compartments=Compartments(intra=Pool(D=3e-9), extra=Pool(D=1e-9))), seed=SEED))
