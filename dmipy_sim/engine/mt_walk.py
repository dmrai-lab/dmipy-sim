...
i.e. the impact-angle rule of :func:`dmipy_sim.engine.mt.stick_probability`, consuming the
...
def simulate_mt_trajectories(
    ...
    n_walkers, diffusivity, geometry, T_max, dt_save, seed :
        As in :func:`dmipy_sim.engine.core.simulate_trajectories`.  ``geometry`` must
    ...
    """
    with Run("simulate_mt_trajectories", params=dict(n_walkers=n_walkers, diffusivity=diffusivity, geometry=type(geometry).__name__)) as run:
        ...
        # the same dispatch every driver uses; this walk records the free-pool local time (surface
        # criterion) and binds (dwell criterion)
        R_geom = _geometry_radius(geometry)
        sub_steps = resolve_sub_steps(geometry, diffusivity, dt_actual, surface=True,
                                      mt_dwell_time=(dwell_time if dwell_time > 0 else None),
                                      override=sub_steps)
        ...
        for s, e in run.batches(n_walkers, walker_batch_size):
        ...
        walk = PersistentWalk(np.concatenate(pos_b, axis=0), float(dt_actual), int(sub_steps), float(dt_sim),
