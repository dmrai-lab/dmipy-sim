...
| `io/` | file readers that construct nothing: `caterpillar.read_caterpillar` (CATERPillar `.csv` / `.swc` by header name, metres), `strands.read_strands` / `write_strands` (EPFL strand list: side, count, per strand `n` then `x y z r`), plus the legacy mesh loaders (`cactus`, `winther`, `mesh_substrate`) that #130 PR 7 retires |
