...
| `acquisition/waveforms.py` | `Waveform` -- **`G` is the PHYSICAL gradient the scanner plays** (a spin echo's lobes share a sign); **`G_eff`** is derived on read as `G * rf_events.sign(t)` and is what the scalar engine, `calc_b`, `calc_btensor`, the pack's `replay` and the q-trace integrate; the vector-Bloch routes read `G` and apply the pulses themselves. Builders `pgse/pgste/ogse/trapezoidal_ogse/cpmg/ste/pte`, `set_b`; **the** b / B-tensor integrals `b_from_gradient`, `btensor_from_gradient` (rectangular q, trapezoidal ∫). Nothing stores an effective gradient (`test_api_surface` locks `G_display` gone) |
| `sequences/` | `Sequence` = a `Waveform` (same readout attributes: physical `G`, derived `G_eff`, `echo_idx`, `echo_indices`, `rf_events`, `chi_perp`, …) plus per-measurement encoding (`bvalues`, `gradient_directions`, `delta`, `Delta`, `TE`, family flags) for dmipy-fit. Every `from_X` constructor declares the pulses it plays, places each row's lobes symmetric about the 180 (a shorter-`Delta` row is shifted, not started at t = 0 with the pulse inside its lobe), scales so `bvalues == b_from_gradient(G_eff, dt)` exactly, and `validate()`s (gradient off across every finite pulse; refocused at the echo); `pulseq` import/export |
