...
| `replay/bank.py` | **build a pack from a walk**: `build_replay_pack(walk, id=, license=, citation=)` assembles every tier the walk carries, reading the pools (C1) and the field basis (C3, myelinated geometries via `field_grid_of`) from `walk.geometry`; `compartments=` overrides, `field=FieldGrid(...)` supplies a mesh basis, `field=False` opts out; no tier booleans and no susceptibility value in the pack (B0, direction, chi are replay knobs). `Substrate.canonical().pack(n_axons=, seed=)` realises the calibrated white matter as a `PackedMyelinatedCylinders` (outer radii from the floored Gamma law, lumens from the g-ratio, pools / rho / kappa set). `ReplayPack.load(path).replay(waveform, rho=, B0=, chi_iso=, chi_aniso=, relaxation=, compartment=)` is the one consume path |
